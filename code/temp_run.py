...
def traverse(root, lines):
    ...
    if len(root.children) == 0:
        ...
        ast_list.append(token) 
        return
    ast_list.append(root.type)

    for child in root.children:
        word_p_id = root.type
        if len(child.children) == 0:
            line = lines[child.start_point[0]]
            word_q_id = line[child.start_point[1] : child.end_point[1]]
        
        else:
            word_q_id = child.type
...
def convert_examples_to_features(js,tokenizer,args):
    ...
    return InputFeatures(source_tokens,source_ids,"ast", js['target'])

def convert_examples_to_features1(js,tokenizer,args):
    #source
    #code=' '.join(js['func'].split())
    #print("Modified convert ex to features ...")
    print("You are in modified ex2fet ... ")
    lines = js["code"].split("\n")
    print("Lines... ", len(lines))
    tree = parser.parse(bytes(js["code"], "utf8"))

    #print("Printing Raw code ...",js["code"])
    #code=' '.join(js['code'].split()) ## original line
    #print("printing code ...", type(code))
    traverse(tree.root_node, lines)
    print("Traverse done ... ... ... ")
    ...
    padding_length = args.block_size - len(source_ids)
    source_ids+=[tokenizer.pad_token_id]*padding_length
    return InputFeatures(source_tokens, source_ids, word_pair_count, js['target'])
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):    
        #print("Inside getitem... ", self.examples[i].ast)   
        return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label), self.examples[i].ast

class TextDatasetMoCo(Dataset):
    def __init__(self, tokenizer, args, file_path=None, sample_percent=1.):
        self.examples = []
        print("Filepath --------------------------xxxxxxxx  MoCO  xxxxxxxx----------------------", file_path)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            model.train()
            loss,logits = model(pair, inputs,labels)
