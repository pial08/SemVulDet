...
def traverse(root, lines, ast_list, word_pair, tokenizer):
    ...
    for child in root.children:
        ...
        #print("original pairs ", word_p_id.split("_")[0], word_q_id.split("_")[0])
        x = word_p_id.split("_")[0] + " " + word_q_id.split("_")[0]
        #print(x)
        #x = tokenizer.convert_tokens_to_ids(x)
        #print(x)
        #print("------",tokenizer.convert_ids_to_tokens([x]), tokenizer.convert_ids_to_tokens([y]))
        ...
        #word_pair_key = " ".join(str(e) for e in word_pair_key)
...
def convert_examples_to_features1(js,tokenizer,args):
    
    ast_list = []   # flattened ast
    word_pair = []  # graph structure stays
    global count_ast_nodes
    #print("You are in modified ex2fet ... ")
    lines = js["code"].split("\n")
    #print("Lines... ", len(lines))
    tree = parser.parse(bytes(js["code"], "utf8"))

    
    #code=' '.join(js['code'].split()) ## original line
    traverse(tree.root_node, lines, ast_list, word_pair, tokenizer)
    #print("AST TOKEN LEN ", count_ast_nodes)
    count_ast_nodes = 0
    #print("Traverse done ... ... ... ")
    #print("Printing word pair count ..................", word_pair_count)
    #call ast parser which returns list
    code = ' '.join(ast_list)
    #print("Len of AST edge pairs ...", len(word_pair))
    code_tokens=tokenizer.tokenize(code)[:args.block_size-2]
    #print(code)
    ...
    #print("-----padding len ", padding_length, " --- original len", len(word_pair))
    if padding_length < 0:
        word_pair = word_pair[: args.block_size]
        #print("after padding if", len(word_pair))
    elif padding_length > 0:
        #temp = [0, 0] * int((padding_length / 2))
        #print("Temp len = ", len(temp))
        word_pair += [0] * padding_length
        #print("after padding else", len(word_pair))
    #source_id is always 40

    #print(word_pair)

    #print("Printing source id and word pair count ",len(source_ids), len(word_pair))
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None, sample_percent=1.):
        ...
        #print("data loading done inside TextDataset ...", total_len)
        #print("---------")
        #print(self.examples[0].input_ids)
        #print(tokenizer.convert_ids_to_tokens(self.examples[0].input_ids))
        #print("--------------", self.examples[0].ast)
        #print(len(self.examples[10].ast))
    ...
    def __getitem__(self, i):
        #print("Inside getitem... ", len(self.examples[i].ast))
        #return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label), self.examples[i].ast
        
        #return  torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label) #self.examples[i].ast )
        return  torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label),  torch.tensor(self.examples[i].ast)   
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            #print("Value of step ...., ", temp1, temp2)
            #print("Printing inputs:   ", inputs.shape)
            #print("Printing labels:   ", labels.shape) 
            #print("AST item --- ", len(batch[2]))
            model.train()
...
def evaluate(args, model, tokenizer,eval_when_training=False):
    ...
    for batch in eval_dataloader:
        #print("printing input shape", batch[0].shape)
        #print("printing label shape", batch[1].shape)
        inputs = batch[0][:400].to(args.device)
...
def main():
    ...
    #model = model.roberta
    #print("Printing model before adding graph layer and model class ..........", model_class)
    print(model)
    ## model
